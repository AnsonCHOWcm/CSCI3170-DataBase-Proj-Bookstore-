...
public class Customer {
	...
	public void OrderCreation() {
		...
		try {
			...
			String psql = "Insert " + "into orders (order_id , o_date , shipping_status , charge , customer_id)"
			...
			while (true) {
				...
				quantity = checker.Quantitychecker();

				psql = "SELECT unit_price , no_of_copies" + "FROM book" + "WHERE ISBN = ?";
				pstmt = con.prepareStatement(psql);
				pstmt.setString(1, ISBN);
				...
				psql = "Insert " + "into ordering (order_id , ISBN ,  quantity)"
						+ "Values (CAST(? AS CHAR(8)) , ? , ?)";
				pstmt = con.prepareStatement(psql);
				pstmt.setInt(1, OID);
				pstmt.setString(2, ISBN);
				...
				psql = "UPDATE book" + "SET no_of_copies = no_of_copies - ?" + "WHERE ISBN = ?";
				pstmt = con.prepareStatement(psql);
				pstmt.setInt(1, quantity);
				pstmt.setString(2, ISBN);
				pstmt.executeUpdate();

				exit = checker.Exitchecker();

				if (exit == "L") {
					int number = 1;
					psql = "SELECT ISBN , quantity" + "FROM ordering" + "WHERE order_id = (CAST(? AS CHAR(8))";
					pstmt = con.prepareStatement(psql);
					pstmt.setInt(1, OID);
					...
					while (rs.next()) {
						...
						System.out.println(
								"ISBN : " + rs.getString("ISBN") + " " + "quantity : " + rs.getInt("quantity"));
						number++;
					}
			...
			String psql2 = "UPDATE orders" + "SET charge = ?" + "WHERE order_id = (CAST(? AS CHAR(8)))";
	...
	public void OrderAltering() {
		...
		try {
			...
			String psql = "SELECT order_id , shipping_status , charge , customer_id" + "FROM orders"
			...
			while (rs.next()) {
				System.out.println("order_id :" + rs.getString("order_id") + " " + "shipping : " + " "
						+ rs.getString("shipping_status") + " " + "charge : " + rs.getInt("charge") + "customerID : "
			...
			while (rs.next()) {
				...
				BookList.add(ISBN);
				System.out
						.println("book no:" + number + "ISBN : " + ISBN + " " + "Quantity : " + rs.getInt("quantity"));
			}

			choice = checker.choicechecker(number);

			ISBN = BookList.get(choice);

			action = checker.Actionchecker();

			if (action == "A") {

				quantity_change = checker.QuantityAddchecker();

				psql = "SELECT shipping_status" + "FROM orders" + "WHERE order_id = (CAST(? AS CHAR(8)))";
				...
				psql = "SELECT no_of_copies" + "FROM book" + "WHERE ISBN = ? ";
				...
				if (shipping_status == "Y") {
					...
					psql = "UPDATE book" + "SET no_of_copies = ?" + "WHERE ISBN = ?";
					...
					psql = "UPDATE ordering" + "SET quantity = quantity + ?" + "WHERE ISBN = ?";
					...
					psql = "SELECT unit_price" + "FROM book" + "WHERE ISBN = ?";
					...
					psql = "UPDATE orders" + "SET o_date = ? , charge = charge + ? "
							+ "WHERE order_id = (CAST(? AS CHAR(8)))";
				...
				psql = "SELECT order_id , shipping_status , charge , customer_id" + "FROM orders"
						+ "WHERE order_id = (CAST( ? AS CHAR(8)))";
				...
				while (rs5.next()) {
					System.out.println("order_id :" + rs5.getString("order_id") + " " + "shipping : " + " "
							+ rs5.getString("shipping_status") + " " + "charge : " + rs5.getInt("charge")
							+ "customerID : " + rs5.getString("customer_id"));
				}

				psql = "SELECT ISBN , quantity" + "FROM ordering" + "where order_id = (CAST(? AS CHAR(8))) , ISBN = ?";
				...
				while (rs5.next()) {
					System.out.println("book no :" + choice + "ISBN :" + rs5.getString("ISBN") + "quantity : "
							+ rs5.getInt("quantity"));
				}

			} else if (action == "D") {

				quantity_change = checker.QuantityDelchecker();
				...
				psql = "SELECT quantity" + "FROM ordering" + "WHERE ISBN = ? ";
				...
				psql = "SELECT ISBN , quantity" + "FROM ordering"
	...
	public void OrderQuery() throws ParseException {
		...
		try {
			...
			year = ft.parse(checker.yearchecker());
